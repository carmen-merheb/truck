...
export function Vehicle({ angularVelocity, children, position, rotation }: VehicleProps) {
  ...
  const { back, force, front, middle, height, maxBrake, steer, maxSpeed, width } = vehicleConfig
  ...
  const wheelInfos = wheels.map((_, index): DerivedWheelInfo => {
    ...
    const sideMulti = index % 2 ? 0.25 : -0.25
    return {
      ...wheelInfo,
...
// Limit steering angle as speed rises (quadratic falloff)
const speedNorm = MathUtils.clamp(speed / maxSpeed, 0, 1)
const steerLimit = MathUtils.lerp(steer, 0.12, speedNorm * speedNorm)

const targetSteer =
  controls.left || controls.right ? steerLimit * (controls.left && !controls.right ? 1 : -1) : 0

steeringValue = lerp(steeringValue, targetSteer, delta * 20)
