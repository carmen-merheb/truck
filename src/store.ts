...
export const wheelInfo: WheelInfo = {
  ...
  // Let it slide before it flips
  frictionSlip: 2.0,          // was ~2.2

  radius: 0.15,               // trucks usually run taller rubber

  // Less body roll
  rollInfluence: 0.02,        // was 0.06

  // Lateral authority (lower = more drift, safer)
  sideAcceleration: 3.2,      // was 4

  // Shorter, stiffer, better damped suspension
  suspensionRestLength: 0.28, // was 0.3
  suspensionStiffness: 60,    // was 45

  // kill pogo/bounce
  dampingCompression: 7.5,    // was 4.8
  dampingRelaxation: 9.5,     // was 6.2
...
export interface IState extends BaseState {
  ...
  wheelInfo: WheelInfo
  wheels: [RefObject<Group>, RefObject<Group>, RefObject<Group>, RefObject<Group>, RefObject<Group>, RefObject<Group>]
  keyInput: string | null

  // --- Checkpoint / respawn ---
  lastCheckpoint: Transform
  setCheckpoint: (cp: Transform) => void
}
...
const useStoreImpl = create<IState>((set: SetState<IState>, get: GetState<IState>) => {
  ...
  return {
    ...
    vehicleConfig,
    wheelInfo,
    wheels: [createRef<Group>(), createRef<Group>(), createRef<Group>(), createRef<Group>(), createRef<Group>(), createRef<Group>()],
...
// always overwrite with the most recent gate you hit
setCheckpoint: (cp) => set(() => ({ lastCheckpoint: cp })),
 // ← always take the last gate hit
